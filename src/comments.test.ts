...
import { createCommentsDb } from './comments'

// ── seed helpers ──────────────────────────────────────────────────────────────

async function seedConversation(tenantAccountId?: number, title?: string) {
  const { data, error } = await admin
    .from('conversations')
    .insert({ tenant_id: tenantAccountId ?? null, type: 'group', title: title ?? null })
    .select()
    .single()
...
describe('conversations RLS', () => {
  ...
  it('participant can view their conversation', async () => {
    const conv = await seedConversation(userA.accountId)
...
describe('createCommentsDb', () => {
  ...
  describe('createConversation / listConversations', () => {
    it('creates a conversation and lists it for a participant', async () => {
      ...
      const list = await userADb.listConversations({ tenantId: userA.accountId })
